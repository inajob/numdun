export const ITEMS = {
  ...
  reveal_one_trap: {
    ...
    maxFloor: Infinity,
    use: function(game) {
      const neighborsToReveal = game.getEightDirectionsNeighbors(game.player.r, game.player.c);
  ...
  reduce_traps: {
    ...
    maxFloor: 10,
    use: function(game) {
      const neighborsForTrapCheck = game.getEightDirectionsNeighbors(game.player.r, game.player.c);
  ...
  ariadnes_thread: {
    ...
    maxFloor: Infinity,
    use: function(game) {
      const path = game.getLineCells(game.player.r, game.player.c, game.exit.r, game.exit.c);
  ...
  detailed_map_of_exit: {
    ...
    use: function(game) {
      if (!game.exitRevealedThisFloor) {
        game.exitRevealedThisFloor = true;
        const neighbors = game.getEightDirectionsNeighbors(game.exit.r, game.exit.c);
...
export const game = {
  ...
  placeTraps: function(trapCount) {
    const forbiddenTrapZones = this.getEightDirectionsNeighbors(this.player.r, this.player.c);
    forbiddenTrapZones.push({ r: this.player.r, c: this.player.c }); // プレイヤーの開始位置も追加
  ...
  isGoalInitiallyVisible: function(startR, startC, exitR, exitC, grid) {
    ...
    while (queue.length > 0) {
      ...
      // Check if exit is a direct neighbor
      const neighbors = this.getEightDirectionsNeighbors(r, c);
  ...
  handleInput: function(key) {
    ...
    if (this.gameState === 'confirm_next_floor') {
      ...
      // ターンを進め、プレイヤー位置のイベントを処理する
      // (gameStateが'playing'のままの即時効果アイテムか、移動した場合)
